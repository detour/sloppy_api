...
(function($) {
	...
	var methods = {
		...
		destroy: 	function() {
				  return this.each( function(){
					...
					t.removeAttr('data-transform-scale');
	...
	jQuery.fn.transformable = function(options,args,args2) {
		...
		if (options.containment===true||options.containment=='parent') {
			options.scalecontain=options.skewcontain=options.rotatecontain=true;
		}
	...
	function enablescale(tf) {
		...
		if (ri.length) {
			...
			tf.append('<div class="transformable-handle-scale transformable-handle ui-icon ui-icon-search" style="right: -1px; bottom: 0px; position: absolute; z-index: 2000;"></div>');
	...
	function scdblclick(e) {
		...
		if (ri.hasClass('ui-resizable-se')) {
			...
			tf.children('.ui-resizable-se').show()
			  .bind('dblclick.transformable',scdblclick);
		}
		return false;
	}
	...
	function enableskew(tf) {
		...
		tf.append('<div class="transformable-handle-skew-h transformable-handle-skew transformable-handle ui-icon ui-icon-arrow-2-e-w" style="left: 50%; top: 0px; position: absolute; z-index: 2000;"></div>');
	...
	function enablerotate(tf) {
		tf.append('<div class="transformable-handle-rotate transformable-handle ui-icon ui-icon-arrowrefresh-1-s" style="right: -1px; top: 0px; position: absolute; z-index: 2000;"></div>');
	...
	jQuery.fn.transformable.drag=function(e,ui) {
		var t=$(this);
		var startoff=t.data('startoff');
		if (!startoff||!startoff.transformed ) return;
	...
	jQuery.fn.relativeOffset=function(rel,options) {
		...
		return this.each (function (){
			var t=$(this);
			t.tOffset({left:relpoint[0].x, top: relpoint[0].y});
		});
	}
	
	jQuery.fn.tOffset = function (options) {
		if (options) {
		   return this.each ( function () {
			var t=$(this);
			//var tm=t.matrixToArray(true);
			var toff=jQuery.fn.tOffset.call(t);
			...
			var xoff=topleft[0].x-toff.left;
			var yoff=topleft[0].y-toff.top;
	...
	function singleLevelTP (points,center,transform) {
